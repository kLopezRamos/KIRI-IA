...
# def predict():
#     try:
#         # 1. Validar que el archivo llegó bajo la clave 'file' (como pusimos en React Native)
#         if 'file' not in request.files:
#             return jsonify({"error": "No se recibió ningún archivo"}), 400
...
#         # 2. Leer los datos de la imagen una sola vez
#         image_data = file.read()

#         # 3. Llamada a Gemini
#         prompt = "Identify the main object. Respond only with the name in English, a single word."
#         response = model.generate_content([
#             prompt,
#             {'mime_type': 'image/jpeg', 'data': image_data}
#         ])
...
#         object_name = response.text.strip()
#         print(f"DEBUG: Gemini detectó: {object_name}")

#         # 4. Generar Audio (gTTS)
...
#         # 5. Respuesta para el iPhone
#         return jsonify({
#             "object": object_name, # Cambié object_en a object para que coincida con tu CameraScreen.tsx
#             "message": "Success"
#         })
